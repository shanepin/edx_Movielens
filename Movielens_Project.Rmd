...
An RMSE greater than or equl to 1 means our typical error is larger than almost a star, which is not good. 
...
The next step is to incorporate the individual User Effects, $b_u$, in to the model, acknowledging that each users inherent bias to mark all films higher or lower. 
...
The RMSE results table tells us that the Regularised Movie and User Effects Model has been the yielded the lowest RMSE and hence has been the most successfull
at achieving our goal. The basic Naive Mean Model has been least successfull and really is not very helpful at achieving our goals.
...
The lowest value of RMSE obbtained via  this model was 0.8654673.
...
# Predict via regularisation, movie and user effect model
# As lambda of 4.5 was shown to be the best Tune for the earlier models, we will use the same on the final_holdout_set.
...
improves upon the benchmarking model’s Naive RMSE of ~1.06 and can be suggested as an recommended model.
